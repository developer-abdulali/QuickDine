...
export default function OwnerAllBookings({
  ...
  const updateStatus = async (
    ...
    try {
      ...
      setBookings((prev) =>
        prev.map((b) => (b._id === bookingId ? original : b)),
      );
      toast.error(
  ...
  const filtered = filterRestaurantId
    ? bookings.filter(
        (b) => String(restaurantIdOf(b)) === String(filterRestaurantId),
      )
    : bookings;
  ...
  const groups = restaurants
    .filter(
      (r) =>
        !filterRestaurantId || String(r._id) === String(filterRestaurantId),
    )
    .map((r) => ({
      restaurant: r,
      items: filtered.filter(
        (b) => String(restaurantIdOf(b)) === String(r._id),
      ),
    }))
    ...
    <div className="space-y-6 text-left">
        ...
        {restaurants.length > 0 && (
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`px-3 py-1.5 text-[10px] font-medium tracking-wider uppercase rounded-sm transition-colors cursor-pointer ${
                !filterRestaurantId
                  ? "bg-primary text-white"
                  : "bg-white border border-outline-variant/30 text-black/55 hover:border-primary"
              }`}
            >
              All Restaurants
            </button>
            {restaurants.map((r) => (
              <button
                key={r._id}
                onClick={() => onFilterChange(r._id)}
                className={`px-3 py-1.5 text-[10px] font-medium tracking-wider uppercase rounded-sm transition-colors cursor-pointer ${
                  filterRestaurantId === r._id
                    ? "bg-primary text-white"
                    : "bg-white border border-outline-variant/30 text-black/55 hover:border-primary"
                }`}
              >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-8">
          {groups.map((g) => (
            <div key={g.restaurant._id} className="space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
                      ...
                      {b.status === "confirmed" && (
                        <div className="flex gap-2">
                          <button
                            onClick={() => updateStatus(b._id, "completed", b)}
                          ...
                          <button
                            onClick={() => updateStatus(b._id, "cancelled", b)}
