    ...
    <div className="space-y-6 text-left">
      ...
      {users.length === 0 ? (
        <div className="bg-white border border-outline-variant/10 p-12 text-center rounded-md">
          <UsersIcon size={32} className="mx-auto text-outline-variant mb-2" />
              ...
              {users.map((u) => (
                <tr key={u._id} className="hover:bg-surface/50">
                  <td className="p-4">
                    ...
                    <div className="text-[10px] text-black/50">{u.email}</div>
                  </td>
                  <td className="p-4 text-black/55">{u.phone || "—"}</td>
                  <td className="p-4">
                    <select
                      value={u.role}
                      onChange={(e) =>
                        onRoleChange(
                          u._id,
                          e.target.value as "user" | "owner" | "admin",
                        )
                      }
                      disabled={btnLoading === `role-${u._id}`}
                  ...
                  <td className="p-4 text-right">
                    ...
                    >
                      <Trash2Icon size={12} />
                      {btnLoading === `del-${u._id}` ? "Deleting..." : "Delete"}
                    </button>
                  </td>
                </tr>
              ))}
